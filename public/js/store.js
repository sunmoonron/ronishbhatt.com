...
export const TAG = 'ronishbhatt.com', CONFIG_D = 'ronishbhatt.com', CSS_D = 'ronishbhatt.com/css';
export const env = { NT: null, SITE: '', PRIMARY: '', BACKUPS: [], get RELAYS() { return [this.PRIMARY, ...this.BACKUPS]; } };
export const init = ({ site, relay, backups, NT }) => { env.SITE = site.toLowerCase(); env.PRIMARY = relay; env.BACKUPS = backups; if (NT) env.NT = NT; };
export const DEFAULT_CFG = { title: 'Ronish Bhatt', chat: true, sections: ['chat', 'about', 'now', 'projects', 'writing', 'courses', 'archive', 'colophon'],
...
const verify = ev => !env.NT || env.NT.verifyEvent(ev); // no crypto yet = trust the baked snapshot; reverify() runs once it loads
...
export const sel = {
  ...
  configEvent: () => store.events.get(`${K.config}:${env.SITE}:${CONFIG_D}`),
  config: () => { try { const e = sel.configEvent(); return e ? { ...DEFAULT_CFG, ...JSON.parse(e.content) } : DEFAULT_CFG; } catch { return DEFAULT_CFG; } },
  cssEvent: () => store.events.get(`${K.config}:${env.SITE}:${CSS_D}`),
  css: () => sel.cssEvent()?.content ?? '',
  articles: type => [...store.events.values()].filter(e => isPage(e) && (!type || tagsOf(e, 't').includes(type))).sort((a, b) => num(tag(a, 'order')) - num(tag(b, 'order')) || pubAt(b) - pubAt(a)),
  section: d => { const e = store.events.get(`${K.article}:${env.SITE}:${d}`); return e && isPage(e) ? e : null; },
...
// ---- snapshot (baked into the HTML) + localStorage cache ----------------------
export function loadSnapshot(snap) {
  for (const e of snap?.events || []) apply(e, { verified: true });
  for (const d of snap?.drafts || []) { const ev = { ...d, pubkey: env.SITE, created_at: d.created_at || 0, sig: '' }; ev.id = 'draft:' + keyOf(ev); apply(ev, { draft: true }); }
}
...
export function connect() {
  ...
  pool.subscribe(env.RELAYS, { authors: [env.SITE], kinds: [K.profile, K.note, K.del, K.article, K.config], limit: 300 },
