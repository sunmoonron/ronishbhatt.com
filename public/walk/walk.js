// walk.js: a Nostr client with no feed. You stand at the mouth of a box. The far wall is a note. The
// side walls are doors with previews of what is through them: a reply, the author, what this answers
// or who answered, newer, older. Flick to walk through the door ahead; its role is always shown, so
// the walk never asks you to choose. A dead end jumps to a random note from someone you follow, or
// from the world, and starts a new trail. One room rendered, five previews, the next room fetched
// ahead: O(1) per step. Identity, keys, relays and the cache are shared with /cube/.
...
// ---- state ----------------------------------------------------------------------------------------
const S = {
  me: store.get('me', null), follows: store.get('follows', []), muted: new Set(store.get('muted', [])), relays: store.get('relays', DEFAULT_RELAYS), langs: store.get('langs', [(navigator.language || 'en').slice(0, 2)]), bookmarks: new Set(store.get('bookmarks', [])),
  events: new Map(), profiles: new Map(), counts: new Map(), mine: new Map(), byAuthor: new Map(), repliesOf: new Map(), world: [], followsNotes: null, tagNotes: new Map(), mentions: [],
  room: null, doors: null, ctx: { type: 'world' }, trail: [], visited: new Set(), pool: [], moving: false, d: 300, subs: new Map(), steps: 0, ingested: 0, seenMentions: store.get('walk.seenMentions', 0),
};
const isMe = pk => !!(S.me && S.me.pk === pk); const isFollow = pk => S.follows.includes(pk); const muted = pk => S.muted.has(pk);
...
// ---- worker messages: only touch the walls when the room they show is affected --------------------
H.cached = ({ events, profiles }) => { for (const p of profiles) S.profiles.set(p.pubkey, p); for (const ev of events) ingest(ev); if (S.room === 'home') renderHome(); resume(); };
const profileBatch = new Set(); let profileRefresh = 0;
H.profile = ({ profile }) => { const old = S.profiles.get(profile.pubkey); if (old && old.t > profile.t) return; S.profiles.set(profile.pubkey, profile); profileBatch.add(profile.pubkey); clearTimeout(profileRefresh); profileRefresh = setTimeout(() => { const pks = visiblePks(); const hit = [...profileBatch].some(pk => pks.has(pk)); profileBatch.clear(); if (hit) { parseCache = new WeakMap(); refreshWalls(); } }, 300); };
H.events = ({ sub, events }) => {
  const tag = S.subs.get(sub); let relevant = false, homeTouched = false;
  for (const ev of events) { const fresh = ingest(ev); if (tag === 'pool' && poolWorthy(ev)) S.pool.push(ev); if (tag === 'mentions' && ev.kind === 1 && !isMe(ev.pubkey)) noteMention(ev); if (!fresh) continue; if (isRelevant(ev)) relevant = true; if (S.room === 'home') homeTouched = true; }
  if (tag === 'pool') poolWaiters.splice(0).forEach(f => f());
  if (tag === 'get') { pending.forEach((f, id) => { if (S.events.get(id)?.created_at) { pending.delete(id); f(S.events.get(id)); } }); relevant = true; }
  if (relevant) scheduleDoors(); if (homeTouched) scheduleHome();
};
H.eose = ({ sub }) => { const tag = S.subs.get(sub); if (tag === 'pool') poolWaiters.splice(0).forEach(f => f()); if (tag && tag.startsWith('author:')) authorWaiters.get(tag)?.splice(0).forEach(f => f()); if (tag && tag.startsWith('tag:')) tagWaiters.get(tag)?.splice(0).forEach(f => f()); if (tag && S.room && S.room !== 'home' && tag === S.room.id) scheduleDoors(); S.subs.delete(sub); };
H.sent = ({ relays }) => { setTimeout(() => toast(relays ? `sent to ${relays} relay${relays === 1 ? '' : 's'}` : 'no relay connected'), 300); };
function isRelevant(ev) { const r = S.room; if (!r || r === 'home') return false; if (ev.pubkey === r.pubkey) return true; const t = ev.kind === 1 || ev.kind === 7 || ev.kind === 6 || ev.kind === 9735 ? replyTarget(ev) : null; if (t === r.id) return true; if (S.ctx.type === 'author' && ev.pubkey === S.ctx.pk) return true; return false; }
function visiblePks() { const set = new Set(); if (S.room && S.room !== 'home') { set.add(S.room.pubkey); for (const d of Object.values(S.doors || {})) { if (d?.ev) set.add(d.ev.pubkey); if (d?.pk) set.add(d.pk); } } return set; }
let doorsTimer = 0; function scheduleDoors() { if (doorsTimer) return; doorsTimer = setTimeout(() => { doorsTimer = 0; refreshDoors(); refreshCounts(); }, 250); }
let homeTimer = 0; function scheduleHome() { if (homeTimer) return; homeTimer = setTimeout(() => { homeTimer = 0; if (S.room === 'home') renderHome(); }, 1000); }
...
function ingest(ev) {
  ...
  S.events.set(ev.id, ev); needProfile(ev.pubkey); if (++S.ingested % 2000 === 0) trimEvents();
  ...
  if (ev.kind === 1 && !target && !botlike(ev) && !muted(ev.pubkey)) { S.world.splice(lower(S.world, ev.created_at), 0, ev); if (S.world.length > 600) S.world.shift(); }
  for (const t of ev.tags) if (t[0] === 't' && typeof t[1] === 'string') { const key = t[1].toLowerCase(); const list = S.tagNotes.get(key); if (list) list.splice(lower(list, ev.created_at), 0, ev); }
  return true;
}
function trimEvents() { const keep = new Set([...S.visited, ...S.trail.map(t => t.id), S.room?.id]); const all = [...S.events.values()].filter(e => e.created_at).sort((a, b) => a.created_at - b.created_at); let drop = Math.max(0, all.length - 9000); for (const e of all) { if (drop <= 0) break; if (keep.has(e.id)) continue; S.events.delete(e.id); const a = S.byAuthor.get(e.pubkey); if (a) { const i = a.indexOf(e); if (i >= 0) a.splice(i, 1); } drop--; } S.world = S.world.filter(e => S.events.has(e.id)); S.followsNotes = null; }
...
const ok = ev => ev && ev.created_at && !S.visited.has(ev.id) && !botlike(ev) && !muted(ev.pubkey);
...
const replies = ev => (S.repliesOf.get(ev.id) || []).map(id => S.events.get(id)).filter(r => r && r.kind === 1 && !botlike(r) && !muted(r.pubkey)).sort((a, b) => score(b) - score(a) || a.created_at - b.created_at);

// ---- content (parsed once per event) -----------------------------------------------------------------------
...
function parse(ev) {
  const hit = parseCache.get(ev); if (hit) return hit;
  ...
  for (const tok of (ev.content || '').split(TOK)) {
    ...
    if (tok[0] === '#' && tok.length > 1) { html += `<span class="tag" data-tag="${esc(tok.slice(1).toLowerCase())}">${esc(tok)}</span>`; plain += ' ' + tok; continue; }
  ...
  const out = { html: html.trim(), plain: plain.replace(/\s+/g, ' ').trim(), media, quotes }; parseCache.set(ev, out); return out;
}
...
function quoteHTML(id) { const q = S.events.get(id); if (!q || !q.created_at) { getEvent(id).then(() => scheduleDoors()); return `<div class="quote" data-quote="${id}">quoted note · loading</div>`; } return `<div class="quote" data-quote="${id}"><b>${esc(name(q.pubkey))}</b> · ${ago(q.created_at)}<div>${esc(parse(q).plain.slice(0, 240))}</div></div>`; }
...
function ctxList() { return S.ctx.type === 'author' ? authorNotes(S.ctx.pk) : S.ctx.type === 'follows' ? followsNotes() : S.ctx.type === 'tag' ? (S.tagNotes.get(S.ctx.tag) || []) : S.ctx.type === 'mentions' ? S.mentions : S.world; }
function ctxNext(ev, dir) { const list = ctxList(); if (!list.length) return null; let i = list.findIndex(e => e.id === ev.id); if (i < 0) i = lower(list, ev.created_at) - (dir > 0 ? 1 : 0); for (let j = i + dir; j >= 0 && j < list.length; j += dir) { const e = list[j]; if (e.id !== ev.id && ok(e)) return e; } return null; }
function latestOf(pk, except) { const a = authorNotes(pk); for (let i = a.length - 1; i >= 0; i--) if (a[i].id !== except && ok(a[i])) return a[i]; return null; }
function doorsFor(ev) {
  ...
  let olderA = null; for (let j = (i < 0 ? lower(mine, ev.created_at) : i) - 1; j >= 0; j--) if (ok(mine[j])) { olderA = mine[j]; break; }
  ...
  const c = S.counts.get(ev.id); let answerer = null; if (c && c.who.size) answerer = [...c.who.entries()].sort((a, b) => b[1] - a[1]).map(x => x[0]).find(pk => pk !== ev.pubkey && !isMe(pk) && !muted(pk));
...
const ROLE = { reply: 'a reply', 'older-author': 'their older note', author: 'the author', parent: 'what this answers', quote: 'the quoted note', person: 'someone who answered', newer: 'newer', older: 'older', jump: 'somewhere new', back: 'back' };
...
function doorHTML(d, side) {
  ...
  const cls = d.kind === 'jump' ? ' jump' : (d.kind === 'newer' || d.kind === 'older') ? ' time' : ''; let body = '';
  ...
  else if (d.ev) { const { plain, media } = parse(d.ev); body = `<div class="w">${avatarHTML(d.ev.pubkey)}<span>${esc(name(d.ev.pubkey))}</span><span class="t">${ago(d.ev.created_at)}</span></div><div class="s">${esc(plain.slice(0, 220) || (media.length ? '[picture]' : '…'))}</div>`; }
...
function renderRoom(el, ev, doors) { el.innerHTML = WALLS.map(w => `<div class="wall ${w}">${w === 'front' ? cardHTML(ev) + aheadHTML(doors.front) : doorHTML(doors[w], w)}</div>`).join(''); }
function refreshDoors() { if (!S.room || S.room === 'home' || S.moving) return; S.doors = doorsFor(S.room); const A = $('roomA'); for (const w of WALLS.slice(1)) { const wall = A.querySelector('.wall.' + w); if (wall) wall.innerHTML = doorHTML(S.doors[w], w); } const ah = A.querySelector('.ahead'); if (ah) ah.outerHTML = aheadHTML(S.doors.front); const front = S.doors.front; if (front?.ev) prefetch(front.ev); hud(); }
function refreshCounts() { if (!S.room || S.room === 'home') return; const c = $('roomA').querySelector('.card .counts'); if (c) c.innerHTML = countsHTML(S.room); }
function refreshWalls() { if (!S.room || S.moving) return; if (S.room === 'home') { renderHome(); return; } refreshDoors(); const card = $('roomA').querySelector('.card'); if (card) { const nm = card.querySelector('.nm'); if (nm) nm.textContent = name(S.room.pubkey); const av = card.querySelector('.av'); const p = prof(S.room.pubkey); if (av && p.pic && !av.querySelector('img')) av.outerHTML = avatarHTML(S.room.pubkey); } }

// ---- moving --------------------------------------------------------------------------------------------------------
const pending = new Map();
function getEvent(id) { return new Promise(res => { const have = S.events.get(id); if (have?.created_at) return res(have); pending.set(id, res); send({ type: 'get', ids: [id] }); setTimeout(() => { if (pending.get(id) === res) { pending.delete(id); res(null); } }, 5000); }); }
function fetchContext(ev) { if (ev.fetched) return; ev.fetched = true; subscribe([{ kinds: [1, 6, 7, 9735], '#e': [ev.id], limit: 150 }, { kinds: [1], authors: [ev.pubkey], limit: 20 }], { tag: ev.id, timeout: 8000 }); const ids = [replyTarget(ev), parse(ev).quotes[0]].filter(id => id && !S.events.get(id)?.created_at); for (const id of ids) getEvent(id).then(() => scheduleDoors()); }
...
const authorWaiters = new Map(); function fetchAuthor(pk) { return new Promise(res => { const tag = 'author:' + pk; if (!authorWaiters.has(tag)) authorWaiters.set(tag, []); authorWaiters.get(tag).push(res); subscribe([{ kinds: [1], authors: [pk], limit: 15 }], { tag, timeout: 6000 }); setTimeout(res, 6500); }); }
...
async function enter(ev, side = 'front', o = {}) {
  if (S.moving || !ev) return; S.moving = true;
  if (o.reset) S.trail = []; else if (S.room && S.room !== 'home' && !o.isBack) S.trail.push({ id: S.room.id, ctx: o.fromCtx || S.ctx });
  S.visited.add(ev.id); S.room = ev; const doors = doorsFor(ev);
  renderRoom($('roomB'), ev, doors); fetchContext(ev);
  if (side !== 'front' && side !== 'back' && !REDUCED) await glance(side);
  await dolly(); S.doors = doors; S.moving = false; refreshDoors(); hud(); trailStrip();
  if (++S.steps > 1) $('hint').classList.add('gone'); try { history.replaceState(null, '', '#' + NT.nip19.noteEncode(ev.id)); } catch {} store.set('walk.last', ev.id);
}
function glance(side) { return new Promise(res => { const rig = $('rig'); const rot = { left: 'rotateY(-16deg)', right: 'rotateY(16deg)', top: 'rotateX(14deg)', bottom: 'rotateX(-14deg)' }[side]; rig.classList.add('glance'); requestAnimationFrame(() => { rig.style.transform = rot; }); setTimeout(() => { rig.classList.remove('glance'); res(); }, 240); }); }
function dolly() { return new Promise(res => { const rig = $('rig'), A = $('roomA'), B = $('roomB'); let done = false; const end = () => { if (done) return; done = true; rig.removeEventListener('transitionend', end); rig.classList.add('snap'); rig.style.transform = ''; A.id = 'roomB'; B.id = 'roomA'; A.className = 'room next'; B.className = 'room'; A.innerHTML = ''; requestAnimationFrame(() => rig.classList.remove('snap')); res(); }; if (REDUCED) { end(); return; } rig.addEventListener('transitionend', end); requestAnimationFrame(() => { A.classList.add('passing'); rig.style.transform = `translateZ(${2 * S.d}px)`; }); setTimeout(end, 800); }); }
function back() { if (S.moving) return; if (!S.trail.length) { toast('this is where the trail starts'); return; } const t = S.trail.pop(); const ev = S.events.get(t.id); if (!ev) return; S.ctx = t.ctx || S.ctx; enter(ev, 'back', { isBack: true }); }
function backTo(i) { if (S.moving || i >= S.trail.length) return; const t = S.trail[i]; S.trail = S.trail.slice(0, i); const ev = S.events.get(t.id); if (!ev) return; S.ctx = t.ctx || S.ctx; enter(ev, 'back', { isBack: true }); }
// the jump: a dead end is never the end
const poolWaiters = []; const poolWorthy = ev => ev.kind === 1 && !replyTarget(ev) && ok(ev) && !isMe(ev.pubkey) && ev.created_at > now() - 30 * 86400;
...
function takeFromPool() { S.pool = S.pool.filter(poolWorthy); if (S.pool.length) return S.pool.splice(Math.floor(Math.random() * S.pool.length), 1)[0]; const local = (S.follows.length ? followsNotes() : S.world).filter(poolWorthy); return local.length ? pick(local.slice(-120)) : null; }
async function jump(side = 'front') { if (S.moving) return; let ev = takeFromPool(); if (!ev) { toast('finding somewhere new…'); await refillPool(); ev = takeFromPool(); } if (!ev) { toast('the relays gave nothing yet; try again in a moment'); return; } S.ctx = { type: S.follows.length ? 'follows' : 'world' }; if (S.pool.length < 6) refillPool(); await enter(ev, side, { reset: true }); }
const tagWaiters = new Map();
async function walkTag(tag) { if (S.moving) return; if (!S.tagNotes.has(tag)) { S.tagNotes.set(tag, [...S.events.values()].filter(e => e.created_at && e.kind === 1 && e.tags.some(t => t[0] === 't' && String(t[1]).toLowerCase() === tag)).sort((a, b) => a.created_at - b.created_at)); const key = 'tag:' + tag; tagWaiters.set(key, []); toast(`walking #${tag}…`); await new Promise(res => { tagWaiters.get(key).push(res); subscribe([{ kinds: [1], '#t': [tag], limit: 40 }], { tag: key, timeout: 6000 }); setTimeout(res, 6500); }); } const list = S.tagNotes.get(tag).filter(ok); if (!list.length) { toast(`nobody has walked #${tag} on these relays`); return; } S.ctx = { type: 'tag', tag }; enter(list[list.length - 1], 'front', { fromCtx: S.ctx }); }
function hud() { $('depth').textContent = S.room === 'home' ? '' : `${S.trail.length + 1} deep`; $('where').textContent = S.room === 'home' ? 'home' : S.ctx.type === 'author' ? `· walking ${name(S.ctx.pk)}'s notes` : S.ctx.type === 'follows' ? '· walking the people you follow' : S.ctx.type === 'tag' ? `· walking #${S.ctx.tag}` : S.ctx.type === 'mentions' ? '· walking your mentions' : '· walking the world'; $('backBtn').disabled = !S.trail.length; }
function trailStrip() { const T = $('trail'); T.innerHTML = S.trail.map((t, i) => `<i title="${esc(name(S.events.get(t.id)?.pubkey))}" data-i="${i}"></i>`).join('') + (S.room && S.room !== 'home' ? '<i class="now"></i>' : ''); }
$('trail').addEventListener('click', e => { const i = e.target.closest('i[data-i]'); if (i) backTo(Number(i.dataset.i)); });
function noteMention(ev) { if (!S.mentions.some(m => m.id === ev.id)) S.mentions.splice(lower(S.mentions, ev.created_at), 0, ev); const n = S.mentions.filter(m => m.created_at > S.seenMentions).length; $('bell').hidden = !n; $('bell').textContent = String(n); }

// ---- home: your cube, and the first minute --------------------------------------------------------------------
function renderHome() {
  S.room = 'home'; S.doors = null; const me = S.me; const A = $('roomA'); A.className = 'room';
  const world = [...S.world].reverse().find(ok), fn = S.follows.length ? [...followsNotes()].reverse().find(ok) : null, own = me?.pk ? latestOf(me.pk) : null, mention = [...S.mentions].reverse().find(ok);
  const doors = { left: fn ? { kind: 'newer', ev: fn } : null, right: mention ? { kind: 'reply', ev: mention } : null, top: world ? { kind: 'newer', ev: world } : null, bottom: own ? { kind: 'older-author', ev: own } : null };
  const front = me ? `<h1>walk</h1><p>you are standing at the mouth of a box. the far wall is a note; the other walls are doors. flick up or press enter to walk through the door ahead. arrows take the others. a dead end jumps you somewhere new.</p><p style="color:#7a6a3c">${esc(me.name || 'you')}${me.pk ? (me.mode === 'read' ? ' · looking around without a key' : ' · with a key') : ' · without a key'} · ${S.follows.length} followed</p><div class="row"><button class="mb pri" data-home="jump">step in</button><a class="mb" href="/cube/">the feed version</a></div>` : `<h1>walk</h1><p>a client with no feed. before you step in, what should people call you?</p><input id="obName" maxlength="40" placeholder="a name" autocomplete="nickname"><div class="row"><button class="mb pri" data-home="look">look around</button><button class="mb" data-home="key">make me a key</button>${window.nostr ? '<button class="mb" data-home="ext">use my extension</button>' : ''}</div><p style="font-size:.85em">a key stays in this browser. it is shared with the feed version at /cube/.</p>`;
  const homeDoor = (d, side, label, empty) => d ? doorHTML(d, side).replace(/<div class="role">.*?<\/div>/, `<div class="role"><i>${ARROW[side]}</i>${label}</div>`) : `<div class="door dim" data-side="${side}"><div class="role"><i>${ARROW[side]}</i>${label}</div><div class="s">${empty}</div></div>`;
  A.innerHTML = `<div class="wall front"><div class="card home">${front}</div></div><div class="wall left">${homeDoor(doors.left, 'left', 'the people you follow', 'follow someone and they appear here')}</div><div class="wall right">${homeDoor(doors.right, 'right', 'your mentions', me?.pk ? 'nobody has mentioned you yet' : 'mentions need a key')}</div><div class="wall top">${homeDoor(doors.top, 'top', 'newest in the world', 'the world is loading')}</div><div class="wall bottom">${homeDoor(doors.bottom, 'bottom', 'your notes', 'nothing of yours yet')}</div>`;
  S.homeDoors = doors; hud(); trailStrip();
}
async function homeAction(what) {
  if (what === 'jump') return jump('front');
  const nm = $('obName')?.value.trim().slice(0, 40) || 'someone'; const me = { name: nm, about: '', pic: '', mode: 'read', pk: null, sk: null };
...
function homeThrough(side) { const d = S.homeDoors?.[side]; if (!d) { toast('nothing this way yet'); return; } S.ctx = side === 'left' ? { type: 'follows' } : side === 'bottom' ? { type: 'author', pk: S.me.pk } : side === 'right' ? { type: 'mentions' } : { type: 'world' }; if (side === 'right') { S.seenMentions = now(); store.set('walk.seenMentions', S.seenMentions); $('bell').hidden = true; } enter(d.ev, side); }
function goHome() { if (S.moving) return; S.trail = []; S.ctx = { type: 'world' }; renderHome(); try { history.replaceState(null, '', location.pathname); } catch {} store.set('walk.last', null); }

// ---- acting --------------------------------------------------------------------------------------------------------
async function sign(t) { const me = S.me; if (!me?.pk || me.mode === 'read') { toast('you are looking around without a key; make one at home'); return null; } const ev = { kind: t.kind, created_at: now(), tags: t.tags || [], content: t.content || '', pubkey: me.pk }; try { if (me.mode === 'nip07') return await window.nostr.signEvent(ev); return NT.finalizeEvent(ev, bytesOf(me.sk)); } catch { toast('signing failed'); return null; } }
...
async function act(what, card) {
  ...
  if (what === 'like') { if (S.mine.get(ev.id)?.liked) return; const r = await publish({ kind: 7, content: '+', tags: [['e', ev.id], ['p', ev.pubkey], ['k', String(ev.kind)]] }); if (r) { refreshCounts(); toast('liked'); } }
  else if (what === 'repost') { if (S.mine.get(ev.id)?.reposted) return; const r = await publish({ kind: 6, content: JSON.stringify(ev), tags: [['e', ev.id], ['p', ev.pubkey]] }); if (r) { refreshCounts(); toast('reposted'); } }
  ...
  else if (what === 'mute') { S.muted.add(ev.pubkey); store.set('muted', [...S.muted]); S.world = S.world.filter(e => e.pubkey !== ev.pubkey); toast(`${name(ev.pubkey)} muted: no more rooms of theirs`); jump('front'); }
  else if (what === 'share') { const url = location.origin + location.pathname + '#' + NT.nip19.noteEncode(ev.id); navigator.clipboard?.writeText(url).then(() => toast('room link copied')).catch(() => toast(url)); }
  else if (what === 'translate') { const box = card.querySelector('.tr'); if (!box.hidden) { box.hidden = true; return; } const { plain } = parse(ev); const from = card.dataset.lang, to = S.langs[0] || 'en'; box.hidden = false; box.innerHTML = `<small>translating from ${esc(from)}…</small>`; if (!REDUCED) card.animate([{ transform: 'rotateY(0)' }, { transform: 'rotateY(360deg)' }], { duration: 650, easing: 'ease-in-out' }); const out = await translate(plain, from, to); box.innerHTML = out ? `<small>translated from ${esc(from)}</small>${esc(out)}` : '<small>translation unavailable right now</small>'; }
}

// ---- input ----------------------------------------------------------------------------------------------------------
document.addEventListener('click', e => {
  const ah = e.target.closest('.ahead'); if (ah) { through('front'); return; }
  const door = e.target.closest('.door'); if (door && !door.classList.contains('dim')) { const side = door.dataset.side; if (S.room === 'home') homeThrough(side); else through(side); return; }
  ...
  const tag = e.target.closest('.tag[data-tag]'); if (tag) { walkTag(tag.dataset.tag); return; }
  const mn = e.target.closest('.mn[data-pk], .nm[data-pk], .av[data-pk]'); if (mn && S.room !== 'home' && S.doors) { const pk = mn.dataset.pk; if (pk === S.room?.pubkey) through('left'); else { S.doors.right = { kind: 'person', pk, ev: latestOf(pk) }; through('right'); } return; }
  ...
  const q = e.target.closest('.quote[data-quote]'); if (q) { const qe = S.events.get(q.dataset.quote); if (qe?.created_at) enter(qe, 'front', { fromCtx: S.ctx }); }
});
...
document.addEventListener('keydown', e => {
  ...
  if (S.room === 'home') { if (k === 'Enter' || k === ' ') { e.preventDefault(); if (S.me) jump('front'); } else if (k === 'ArrowLeft') homeThrough('left'); else if (k === 'ArrowRight') homeThrough('right'); else if (k === 'ArrowUp') { e.preventDefault(); homeThrough('top'); } else if (k === 'ArrowDown') { e.preventDefault(); homeThrough('bottom'); } else if (k === 'r') jump('front'); return; }
...
// wheel: one gesture is one step. Deltas accumulate until a threshold, then lock until the wheel goes quiet
let wheelAcc = 0, wheelLock = false, wheelQuiet = 0;
document.addEventListener('wheel', e => { const c = e.target.closest('.card'); if (c && c.scrollHeight > c.clientHeight + 4 && !((e.deltaY > 0 && c.scrollTop + c.clientHeight >= c.scrollHeight - 2) || (e.deltaY < 0 && c.scrollTop <= 0))) return; clearTimeout(wheelQuiet); wheelQuiet = setTimeout(() => { wheelLock = false; wheelAcc = 0; }, 320); if (wheelLock) return; wheelAcc += e.deltaY; if (Math.abs(wheelAcc) < 110) return; wheelLock = true; const dir = wheelAcc > 0 ? 1 : -1; wheelAcc = 0; if (S.room === 'home') { if (dir > 0 && S.me) jump('front'); return; } if (dir > 0) through('front'); else back(); }, { passive: true });
let touch = null; document.addEventListener('touchstart', e => { if (e.touches.length === 1) touch = { x: e.touches[0].clientX, y: e.touches[0].clientY, card: e.target.closest('.card') }; }, { passive: true });
...
// ---- geometry: you stand just outside the mouth; landscape fits the mouth, portrait fits the far wall ----------------
function size() {
  const vw = innerWidth, vh = innerHeight, vmin = Math.min(vw, vh), portrait = vh > vw;
  const P = Math.max(600, Math.min(1400, Math.round(vmin * 1.2)));
  const m = Math.round(P * (portrait ? 0.02 : 0.06)), d = Math.round(P * (portrait ? 0.18 : 0.3));
  const sNear = P / (P + m), sFar = P / (P + m + 2 * d), sMid = P / (P + m + d);
  const w = portrait ? Math.round(vw * 0.44 / sFar) : Math.round(vw * 0.45 / sNear), h = portrait ? Math.round(vh * 0.40 / sFar) : Math.round(vh * 0.44 / sNear);
  S.d = d; const r = document.documentElement.style; r.setProperty('--p', P + 'px'); r.setProperty('--m', m + 'px'); r.setProperty('--d', d + 'px'); r.setProperty('--w', w + 'px'); r.setProperty('--h', h + 'px'); r.setProperty('--scale', (1 / sFar).toFixed(3)); r.setProperty('--side', (1 / sMid).toFixed(3));
}
addEventListener('resize', size); size();
function arrive() { const rig = $('rig'); if (REDUCED) { rig.classList.remove('far'); return; } rig.classList.add('arrive'); setTimeout(() => rig.classList.remove('far'), 60); setTimeout(() => rig.classList.remove('arrive'), 1500); }
function bootMe() { if (!S.me?.pk) return; subscribe([{ kinds: [3], authors: [S.me.pk], limit: 2 }], { timeout: 8000 }); subscribe([{ kinds: [1], '#p': [S.me.pk], since: now() - 7 * 86400, limit: 40 }], { tag: 'mentions', live: true, id: 'mentions' }); needProfile(S.me.pk); }
let resumed = false;
async function resume() { if (resumed) return; resumed = true; const hash = location.hash.slice(1); let id = null; if (/^(note1|nevent1)/.test(hash)) { try { const dd = NT.nip19.decode(hash); id = dd.type === 'note' ? dd.data : dd.data.id; } catch {} } if (!id && S.me) id = store.get('walk.last', null); if (!id) return; const ev = await getEvent(id); if (ev && !S.moving && S.room === 'home') { S.ctx = { type: 'world' }; enter(ev, 'front', { reset: true }); } }
send({ type: 'start', relays: S.relays, profileRelays: ['wss://purplepag.es'] });
subscribe([{ kinds: [1], limit: 40 }], { live: true, tag: 'live', id: 'live' });
bootMe(); renderHome(); refillPool(); arrive();
window.walk = { S, through, jump, back, enter, goHome, doorsFor, renderHome, walkTag, size };
